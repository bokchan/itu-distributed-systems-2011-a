...
public class DbAdapter {
     ...
     * 
     * 
     * Previously we had time_stamp_start integer not null default current_timestamp
     * but current_timestamp results in the GMT time. for local time we use:
     * (datetime('now','localtime')) don't do datetime('now','localtime') the () are crusial.
     * 
     * If you are in doubt fire up a empty sqlite db and fire of some tests queries.
     * 
     * 
     */
    private static final String DATABASE_CREATE_MOTIONS =
        "create table motions (_id integer primary key autoincrement, "
    ...
    private static final String DATABASE_CREATE_ACCELEROMETER =
    		"create table accelerometer (time_stamp integer not null default (datetime('now','localtime')),"
...
CREATE TABLE accelerometer (time_stamp integer not null default current_timestamp,x real not null, y real not null, z real not null);
CREATE TABLE android_metadata (locale TEXT);
CREATE TABLE motions (
_id integer primary key autoincrement, 
uuid text not null, 
time_stamp_start integer not null default current_timestamp,time_stamp_end integer, motion_type text not null,title text, description text, sync_flag integer, city text, country text);
sqlite> 
...
//    public synchronized void close() {
     ...
     * raw sqlite insert:
     * 
     * insert into motions (uuid, motion_type) values ("19e20a70-8690-40db-bcb5-585ee0c154df", "running");
	 * insert into motions (uuid, motion_type) values ("19e20a70-8690-40db-bcb5-585ee0c154df", "running");
     ...
     * End motion
     * 
     * Raw sqlite in use (example):
     * UPDATE motions SET time_stamp_end=current_timestamp WHERE uuid="020b5eb3-6d50-41c0-893a-0a31d8332bb1";
     * 
     * @param UUID
     * @return
     */
    ...
    public void endMotion(String UUID) {
    	...
    	// Note we dont use java/android sqlite approach since it cant handle insert / update sqlite time stamp - based upon:
...
//      // note: UUID should be compared as string  and for that reason surrounded with ''
//      return mDb.update(DATABASE_TABLE_MOTIONS, args, KEY_UUID + "=" + "'" + UUID + "'", null) > 0;
        
    	// Raw sqlite in use - remember to sourround UUID with ''
    	String sql = "UPDATE "+DATABASE_TABLE_MOTIONS+" SET time_stamp_end=(datetime('now','localtime')) WHERE uuid='"+UUID+"'"; 
    	Log.d(TAG, "sql: " + sql);
        mDb.execSQL(sql);
...
//    public synchronized void createMotionPoint(double x, double y, double z ) {
    public void createMotionPoint(double x, double y, double z ) {
        ...
        // Quick fix:
        // - some times the service has closed down database and the sensor event tries to make insert - perhaps an issue of concurrency ?
        // trace back: http://pastebin.com/XxffJdJt - was though perhaps related to the fact that we did some heavy logging of time etc ?
        try {
