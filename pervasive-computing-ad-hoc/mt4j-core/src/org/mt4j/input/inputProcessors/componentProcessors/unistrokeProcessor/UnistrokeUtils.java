...
/**
 * The Class MTDollarUtils, all calculations for the Gesture Recognizer Based on
 * the Code from http://depts.washington.edu/aimgroup/proj/dollar/
 * http://www.openprocessing.org/visuals/?visualID=600
 */
public class UnistrokeUtils implements Serializable{
	...
	/** The Half diagonal. */
	private final float HalfDiagonal = (float) (0.5 * Math
	...
	private final float Phi = (float) (0.5 * (-1.0 + Math.sqrt(5.0))); // Golden
																		// Ratio
	 ...
	 */
	public enum UnistrokeGesture implements Serializable{
	...
	public class Recorder {
		...
		public void record(List<Vector3D> points) {
			...
			for (Vector3D point : points) {
				if (position < 4) {
					position++;
					System.out.print("new Vector3D(" + (int) point.getX() + ","
							+ (int) point.getY() + "),");
				} else {
					position = 1;
					System.out.println("new Vector3D(" + (int) point.getX()
							+ "," + (int) point.getY() + "),");
				}
	...
	public class Recognizer implements Serializable {
		...
		public Template addTemplate(UnistrokeGesture gesture,
				List<Vector3D> points, Direction direction) {
			Template custom =  dollarTemplates.new Template(gesture, points, direction);
			dollarTemplates.templates.add(custom);
			return custom;
		}
		
		
		public List<Template> getDollartemplates() {
			return this.dollarTemplates.templates;
		}
		...
		UnistrokeGesture Recognize(List<Vector3D> points) {
			...
			for (Template template : Templates) {
				float d = DistanceAtBestAngle(points, template,
						-getAngleRange(), getAngleRange(), getAnglePrecision());
				if (d < best) {
					sndBest = best;
					best = d;
					g = template.gesture;
	...
	List<Vector3D> Resample(List<Vector3D> points, int n, Direction dir) {
		...
		while (!stack.isEmpty()) {
			...
			if ((D + d) >= I) {
				float qx = pt1.getX() + ((I - D) / d)
						* (pt2.getX() - pt1.getX());
				float qy = pt1.getY() + ((I - D) / d)
						* (pt2.getY() - pt1.getY());
	 ...
	 */
	float DistanceAtBestAngle(List<Vector3D> points, Template T, float a,
			float b, float threshold) {
	...
	List<Vector3D> RotateBy(List<Vector3D> points, float theta) {
		...
		List<Vector3D> newpoints = new ArrayList<Vector3D>();
		for (Vector3D point : points) {
			float qx = (point.getX() - c.getX()) * Cos
					- (point.getY() - c.getY()) * Sin + c.getX();
			float qy = (point.getX() - c.getX()) * Sin
					+ (point.getY() - c.getY()) * Cos + c.getY();
			newpoints.add(new Vector3D(qx, qy));
		}
		return newpoints;
	}
	...
	List<Vector3D> RotateToZero(List<Vector3D> points) {
		...
		Vector3D c = Centroid(points);
		float theta = (float) Math.atan2(c.getY() - points.get(0).getY(),
